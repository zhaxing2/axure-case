...
(function (window, undefined) {
  ...
  function scaleContent($page){
  	if(jimUtil.scale && jimUtil.scale!=100){
  ...
  function handleNavigation(target, args) {
	...
	if(target && typeof(target)==="string") {
	  ...
	  if(!target.match(/screens\/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}/)) {
  ...
  function changeInputType() {
	  $('#simulation').find('input[type="date"], input[type="time"], input[type="datetime-local"]').each(function() {
