...
$axure.internal(function($ax) {
    ...
    var _refreshRepeater = function(repeaterId, eventInfo, itemsPregen) {
        ...
        var orderedIds = itemsPregen ? [] : getOrderedIds(repeaterId, eventInfo);
        var ids = [];
        ...
        var oldWidth = $ax.getNumFromPx($jobj(repeaterId).css('width'));
        var oldHeight = $ax.getNumFromPx($jobj(repeaterId).css('height'));
        ...
        if (itemsPregen) {
            ...
            var baseIds = [];
            for(i = 0; i < unprocessedBaseIds.length; i++) {
...
//        }
        ...
        var newWidth = $ax.getNumFromPx($jobj(repeaterId).css('width'));
        var newHeight = $ax.getNumFromPx($jobj(repeaterId).css('height'));
    ...
    var _pushItems = _repeaterManager.pushItems = function (repeaterId, itemId, delta, vertical, suppressFire) {
        ...
        if (!wrap && vert != vertical) {
            ...
            if(repeaterDelta) {
                repeaterObj.css(prop, $ax.getNumFromPx(repeaterObj.css(prop)) + repeaterDelta);
                $ax.dynamicPanelManager.fitParentPanel(repeaterId);
            }
        ...
        if (vert == vertical) {
            ...
            repeaterObj.css(prop, $ax.getNumFromPx(repeaterObj.css(prop)) + delta);
            $ax.dynamicPanelManager.fitParentPanel(repeaterId);
        }
...
$axure.internal(function($ax) {
    ...
    _dynamicPanelManager.initFitPanels = function(query) {
        ...
        query.each(function(obj, elementId) {
            if ($ax.public.fn.IsDynamicPanel(obj.type) && obj.fitToContent) fitToContent[fitToContent.length] = elementId;
        });
    ...
    var _getPanelJobj = _dynamicPanelManager.getPanelJobj = function(id) {
        ...
        var container = $jobj($ax.repeater.applySuffixToElementId(id, '_container'));
    ...
    var _getShownState = _dynamicPanelManager.getShownState = function(id) {
        ...
        var jobj = container.length ? container : $jobj(id);
        var children = jobj.children();
        for(var i = 0; i < children.length; i++) {
            var child = children[i];
            while(child.id.indexOf('_container') != -1) child = $jobj(child.id).children()[0];
    ...
    var getContainerSize = function(containerId) {
        ...
        for(var i = 0; i < children.length; i++) {
            ...
            // Ignore fixed
            if ($ax.public.fn.IsDynamicPanel(childObj.type) && childObj.fixedHorizontal) continue;
    ...
    var _compress = function (id, vert, threshold, delta, easing, duration, clamp) {
        ...
        while (obj && ($ax.public.fn.IsReferenceDiagramObject(obj.type) || $ax.public.fn.IsLayer(obj.type))) {
        ...
        if($ax.getTypeFromElementId(id) == $ax.constants.LAYER_TYPE) {
            clamp = [$ax('#' + id)[clampProp]()];
            clamp[1] = clamp[0] + $ax('#' + id)[clampOffset]();
        } else if (!clamp) {
        ...
        var childrenQueues = [$(parent ? '#' + parent : '#base').children()];
        for(var childrenIndex = 0; childrenIndex < childrenQueues.length; childrenIndex++) {
            var children = childrenQueues[childrenIndex];
            for (var i = 0; i < children.length; i++) {
                var child = $(children[i]);
                if($ax.getTypeFromElementId(child.attr('id')) == $ax.constants.LAYER_TYPE) {
                    childrenQueues.push(child.children());
                    continue;
                }
        ...
        var parentPanel = getParentPanel(id);
        if(parentPanel && $ax.getParentRepeaterFromElementId(parentPanel.parent) == repeaterId) return;
    ...
    var _parentHandlesStyles = function(id) {
        ...
        for(var i = 0; i < parents.length; i++) {
            ...
            parentId = $ax.repeater.getScriptIdFromElementId(parentId);
            parentId = parentId.split('_')[0];
            if(itemId) parentId = $ax.repeater.createElementId(parentId, itemId);
            var parentObj = $obj(parentId);
            if (!$ax.public.fn.IsDynamicPanel(parentObj.type) && !$ax.public.fn.IsLayer(parentObj.type)) continue;
