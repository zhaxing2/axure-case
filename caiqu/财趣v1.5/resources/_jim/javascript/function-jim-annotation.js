...
(function(window, undefined) {
  ...
  function updateCurrentUser(authorName, authorEmail, authorID, hasPicture){
	...
	if(!hasPicture){
		...
		newSrc = './resources/_jim/images/sidepanel/user_picture_mini_blk.png';
	}
	else{
	...
	$("#user-image").attr('src',newSrc);
	
	if(!hasPicture){
		newSrc = './resources/_jim/images/sidepanel/user_picture_mini.png';
	}
    ...
    if (comments && comments.length>0) {
      ...
      for(c=0, len=comments.length; c<len; c+=1) {
        ...
        if (comment.parentCode) {
		  ...
		  //new code
		  var elementID = comment.canvasID + "/" +comment.elementID;
  ...
  function getCanvasID($element) {
    return getCanvasIDFromString($element.attr("id"));
  }
  ...
  if(active){
	...
	$("#bottomSeparationLine").addClass("active");
	$("#topSeparationLine").addClass("active");
	$('#commentscontrol').addClass("active");
	$jimBody.css('bottom','8px');
	/*var top = parseInt($jimBody.css('top'));*/
	/*$jimBody.css('top',top + 5 + 'px');*/
	
	$(".comments-separator").css('background-color','#FFFFFF');
	...
	$("#topSeparationLine").removeClass("active");
	$("#bottomSeparationLine").removeClass("active");
	$('#commentscontrol').removeClass("active");
    $jimBody.css('bottom','0px');
    /*var top = parseInt($jimBody.css('top'));*/
    /*$jimBody.css('top',top - 5 + 'px');*/
	
	$(".comments-separator").css('background-color','');
  ...
  function createCommentIcons(){
	...
	$(".groupComments").each(function(index){
		...
		if(elem.length<=0)/*inside data grid*/
			elem = jQuery("#r0_"+elementID);
		elem.each(function(index2){
			if(jimUtil.hasCanvas(canvasID)){
				/*find element location*/
				if(balloonElem.length==0){
					balloonElem = createMarker(canvasID,elementID,jimComments.itemsArray.indexOf(fullElementID)+1);
					elem = findMarkedElement(elem);
					elem.before(balloonElem);
				}
				var top=0;
				var left=0;
				if(elem.css('top')=="auto" || elem.css('left')=="auto" || elem.css('top')=="" || elem.css('left')=="" || elem.parent("td.layout").length>0){
					/*horizontal and vertical layouts*/
					top = parseInt(elem[index2].getBoundingClientRect().top - elem[index2].parentNode.getBoundingClientRect().top) - parseInt($(balloonElem).css('height'))+10;
					left = parseInt(elem[index2].getBoundingClientRect().left - elem[index2].parentNode.getBoundingClientRect().left) - parseInt($(balloonElem).css('width'))+10;
				}
				else{
					/*free layouts*/
					top = parseInt(elem.css('top')) - parseInt($(balloonElem).css('height'))+10;
					left = parseInt(elem.css('left')) - parseInt($(balloonElem).css('width'))+10;
				}
				if(top<0)top=0;
				if(left<0)left=0;
  ...
  function updateCommentUserImages(authorID, newSrc, hasPicture){
	$(".groupComments div.comment input[id='userID'][value='"+authorID+"']").each(function(index){
		$(this).closest(".comment").find("img.user-comment-img").attr('src',newSrc);
	});
	$(".replier").find("img.user-comment-img").attr('src',newSrc);
	hasPicture ? $("span.user-comment-img-add").hide() : $("span.user-comment-img-add").show();		
  }
  ...
  function openUserDialog(){
	...
	$("#sidepanel #user-image").hasClass("nopicture") ? $userDialog.find("#dialog-user-img").attr("src","./resources/_jim/images/sidepanel/nopicture_icon.png") : $userDialog.find("#dialog-user-img").attr("src",$("#sidepanel #user-image").attr("src"));
  ...
  function saveAction(newComment, $form) {
    ...
    $errorElement = (fromDialog) ? $dialog.next() : $form.parents(".editor").find("textarea:visible").closest(".replier");
